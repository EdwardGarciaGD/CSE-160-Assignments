...
function drawUV3DTriangle(vertices, uv) 
{
    var n = 3; //Num of vertices
    var v = new Float32Array(vertices);

    // Buffer Object For Positions
    var bufferVertex = gl.createBuffer();
    if (!bufferVertex) 
    {
        console.log('Failed to create the buffer object');
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, uvBuffer); // Bind the buffer object to target
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(uv), gl.DYNAMIC_DRAW); // Write date into the buffer object
    gl.vertexAttribPointer(a_UV, 2, gl.FLOAT, false, 0, 0); // Assign the buffer object to a_UV variable
    gl.enableVertexAttribArray(a_UV); // Enable the assignment to a_UV variable

    
    gl.drawArrays(gl.TRIANGLES, 0, n); // Draw triangle with num of vertices
}

function drawUVNormal3DTriangle(vertices, uv, normals) 
{
    var n = vertices.length/3; //Num of vertices
    var v = new Float32Array(vertices);
    var norm = new Float32Array(normals);
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, normBuffer); // Bind the buffer object to target
    gl.bufferData(gl.ARRAY_BUFFER, norm, gl.DYNAMIC_DRAW); // Write date into the buffer object
    gl.vertexAttribPointer(a_Normal, 3, gl.FLOAT, false, 0, 0); // Assign the buffer object to a_Normal variable
    gl.enableVertexAttribArray(a_Normal); // Enable the assignment to a_Normal variable
